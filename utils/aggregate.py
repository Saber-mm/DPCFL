...
def update_model(central_model, models, weights=None):
    avg = avg_models(models, weights=weights)
    assign_model(central_model, avg)
    return

def central_update(models, weights=None):   # FedAvg
    avg = avg_models(models, weights=weights)
    # assign_models(models, avg)
    return avg
...
def PCA_proj(M, n_components=2):
    ...
    principal = PCA(n_components = n_components)
    principal.fit(M_scaled)
    M_projected = principal.transform(M) # M_projected has shape (n,d), we transpose it
    return M_projected.T
...
def clusters_to_weights(clusters, num_clients):
    
    K = len(clusters)
    idxs = []
    for i in range(num_clients):
        idx_i = [j for j in range(K) if i in clusters[j]]
        idxs.append(idx_i[0])
    weights = [1/(K * len(clusters[i])) for i in idxs]
            
    return weights


def clusters_to_probs(clusters, num_clients):
...
def count(clusters, num_evens):
    idx = [0 if len(clusters[0]) < len(clusters[1]) else 1]
    idx = idx[0]
    count = 0
...
def sample_cluster(client_probs):
    num_clusters = len(client_probs)
    r = random.random()
    for i in range(num_clusters):
        if i==0:
            ...
            if r>np.sum(client_probs[0:i]) and r<=np.sum(client_probs[0:i+1]):
                return i

def update_gammas(clusters, gammas):
    
    K = len(clusters)
    clusters_size = [len(clusters[i]) for i in range(len(clusters))]
    num_clients = sum(clusters_size)
    minority_idx = np.argmin(clusters_size)
    
    idxs = []
    for i in range(num_clients):
        idx_i = [j for j in range(K) if i in clusters[j]]
        idxs.append(idx_i[0])
    gamma = [1/(K * len(clusters[i])) for i in idxs]

    if len(gammas) == 0:
        gammas = [[gamma[i]] for i in range(num_clients)]    
    else:
        for i in range(num_clients):
            gammas[i].append(gamma[i])
            
    return gammas 


def HC_cluster(M, num_clusters):
    num_clients = M.shape[1]
    clusters = list([i] for i in range(num_clients))
    while len(clusters) > num_clusters:
        idx1 , idx2 = find_closests(clusters, M)
        clusters[idx1] = clusters[idx1] + clusters[idx2]
        clusters.pop(idx2)
    return clusters
...
def GMM(mat, num_clusters, seed):
    
    gm = GaussianMixture(n_components=num_clusters, tol=0.000001, random_state=seed, covariance_type='spherical', init_params = "k-means++")
    gm.weights_init = np.ones(num_clusters)/num_clusters
    gm.fit(mat.T)
    print('Number of EM iterations for fitting the GMM: {}'.format(gm.n_iter_))
    gm.weights_ = np.ones(num_clusters)/num_clusters
    
    probs = gm.predict_proba(mat.T) # probs has shape (args.num_clients, args.num_clusters)
    print('predicted clusters from GMM on the input matrix: {}'.format(gm.predict(mat.T)))
    
    
    return probs, gm.means_.T, np.sqrt(gm.covariances_)


def GMM_overlap(Means, radii):
    ...
    for i in range(N):
        for j in range(i + 1, N):
            ...
            sepscores.append( d / (2 * max(radii[i], radii[j])) )
    overlaps = [1 - special.erf(s/np.sqrt(2)) for s in sepscores]
    
    return sepscores, overlaps